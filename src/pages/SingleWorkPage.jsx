import React from 'react';
import { useParams } from 'react-router-dom';
import { useEffect, useState } from 'react';
import { restBase } from '../utilities/utilities';
import { Tabs, Tab, TabList, TabPanel } from 'react-tabs';
import Slider from "react-slick";
import ProgressBar from 'react-scroll-progress-bar';
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import Loading from '../components/Loading';




function SingleWorkPage() {
 const {slug} = useParams();

  const restPathPosts = restBase + `fp-single-work/?slug=${slug}&acf_format=standard`;
  const [restDataSingleWork, setDataSingleWork] = useState([]);
  const [isLoaded, setIsLoaded]     = useState(false);

  

  // carousel settings
  const settings = {
    dots: false,
    infinite: true,
    speed: 800,
    autoplay: true,
    autoplaySpeed:9000,
    slidesToShow: 1,
    slidesToScroll: 1,
    focusOnSelect: true,
    centerMode: true,
    centerPadding: "150px",
    responsive: [
      {
        breakpoint: 900,         
        settings: {
          dots: true,
          slidesToShow: 1,
          centerPadding: "120px", 
        }
      },
      {
        breakpoint: 800,        
        settings: {
          dots: true,
          slidesToShow: 1,
          centerPadding: "90px", 
        }
      },
      {
        breakpoint: 480,         
        settings: {
          dots: true,
          slidesToShow: 1,
          centerPadding: "60px", 
        }
      },

    ]
  };

  useEffect(() =>{
    const fetchData = async () =>{
      const response_posts = await fetch(restPathPosts);
      if(response_posts.ok){
        const singleWorkData = await response_posts.json();
        setDataSingleWork(singleWorkData);
        setTimeout(() =>{
          setIsLoaded(true);
        },2000)
      }else{
        setIsLoaded(false);
      }
    }
    fetchData();
  },[restPathPosts])
  const gitHubIcon = <svg width="800px" height="800px" viewBox="0 0 22 22" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    
        <title>github [#142]</title>
        <desc>Created with Sketch.</desc>
        <defs>

      </defs>
        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
            <g id="Dribbble-Light-Preview" transform="translate(-140.000000, -7559.000000)" fill="#000000">
                <g id="icons" transform="translate(56.000000, 160.000000)">
                    <path d="M94,7399 C99.523,7399 104,7403.59 104,7409.253 C104,7413.782 101.138,7417.624 97.167,7418.981 C96.66,7419.082 96.48,7418.762 96.48,7418.489 C96.48,7418.151 96.492,7417.047 96.492,7415.675 C96.492,7414.719 96.172,7414.095 95.813,7413.777 C98.04,7413.523 100.38,7412.656 100.38,7408.718 C100.38,7407.598 99.992,7406.684 99.35,7405.966 C99.454,7405.707 99.797,7404.664 99.252,7403.252 C99.252,7403.252 98.414,7402.977 96.505,7404.303 C95.706,7404.076 94.85,7403.962 94,7403.958 C93.15,7403.962 92.295,7404.076 91.497,7404.303 C89.586,7402.977 88.746,7403.252 88.746,7403.252 C88.203,7404.664 88.546,7405.707 88.649,7405.966 C88.01,7406.684 87.619,7407.598 87.619,7408.718 C87.619,7412.646 89.954,7413.526 92.175,7413.785 C91.889,7414.041 91.63,7414.493 91.54,7415.156 C90.97,7415.418 89.522,7415.871 88.63,7414.304 C88.63,7414.304 88.101,7413.319 87.097,7413.247 C87.097,7413.247 86.122,7413.234 87.029,7413.87 C87.029,7413.87 87.684,7414.185 88.139,7415.37 C88.139,7415.37 88.726,7417.2 91.508,7416.58 C91.513,7417.437 91.522,7418.245 91.522,7418.489 C91.522,7418.76 91.338,7419.077 90.839,7418.982 C86.865,7417.627 84,7413.783 84,7409.253 C84,7403.59 88.478,7399 94,7399" id="github-[#142]">

      </path>
                </g>
            </g>
        </g>
      </svg>

      const liveSiteIcon = 
      <svg width="800px" height="800px" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M21 9.00001L21 3.00001M21 3.00001H15M21 3.00001L12 12M10 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V16.2C3 17.8802 3 18.7202 3.32698 19.362C3.6146 19.9265 4.07354 20.3854 4.63803 20.673C5.27976 21 6.11984 21 7.8 21H16.2C17.8802 21 18.7202 21 19.362 20.673C19.9265 20.3854 20.3854 19.9265 20.673 19.362C21 18.7202 21 17.8802 21 16.2V14" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
 

  return (
    <>
    <ProgressBar  height="5" bgcolor='#f08a4b' duration='0.2'/>
    <main className='single-work-main'>
      {
        isLoaded?
        
        (
      <div>
            <section className='single-page-landing'>
              <h1>{restDataSingleWork[0].acf.single_page_title}</h1>
              <img src={restDataSingleWork[0].acf.single_page_work_image.url} alt={restDataSingleWork[0].acf.single_page_work_image.alt} className='featured-image'/>
              <div className='single-page-buttons'>
                <a href={restDataSingleWork[0].acf.live_site_url}>
                  <p>Live Site</p>
                  {liveSiteIcon}  
                </a>
                <a href={restDataSingleWork[0].acf.github_url}>
                  <p>GitHub</p>
                  {gitHubIcon}
                </a>
              </div>
            </section>
            <section className='detailed-section'>
              <article className='overview'>
                <h2>Overview</h2>
                <p>{restDataSingleWork[0].acf.overview_text}</p>
              </article>
              <article className='work-info'>
                <h3>Tools</h3>
                <div className='work-tools'>
                {
                    restDataSingleWork[0].acf.single_work_tools.map(tool=> 
                      <p>{tool}</p> 
                    )
                }
                </div>
              <Tabs>
                  <TabList>
                    <Tab>Highlights</Tab>
                    <Tab>Insights</Tab>
                  </TabList>
                  <TabPanel>
                    {
                      restDataSingleWork[0].acf.highlight_features.map(feature=>{
                          return(
                            <div className='tab-panel-wrapper'>
                              <h4>{feature.feature_title}</h4>
                              <p>{feature.feature_text}</p>
                            </div>
                          )
                       }  
                      )
                    }
                  </TabPanel>

                  <TabPanel>
                  {
                      restDataSingleWork[0].acf.insight_points.map(feature=>{
                          return(
                            <div className='tab-panel-wrapper'>
                              <h4>{feature.insight_title}</h4>
                              <p>{feature.insight_text}</p>
                            </div>
                          )
                       }  
                      )
                    }
                  </TabPanel>
              </Tabs>
              </article>
              <article className='work-gallery'>
                <Slider {...settings}>
                    {
                      restDataSingleWork[0].acf.single_work_image_gallery.map(image=>
                      {
                          if(restDataSingleWork[0].acf.single_page_title == 'ClickFlicks Movie Database'){
                            return(
                              <div className='media-wrapper'>
                                <video muted autoPlay loop>
                                    <source src={image.url} type='video/mp4'/>
                                </video>
                              </div>
                            )
                          } else{
                            return(
                              <div className='media-wrapper'>
                                <img src={image.url} alt={image.alt}/>
                              </div>
                            )
                          }
                     }
                      )
                    }
                </Slider>
              </article>

            </section>
      </div>
        )

     
        : 
        <Loading/>
      }   
    </main>
    </>
  )
}

export default SingleWorkPage